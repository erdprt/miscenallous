...
public class MapFormat extends Format {
	...
	/**
	 * Designated method. It gets the string, initializes HashFormat object and returns converted string. It scans <code>pattern</code> for {}
	 * brackets, then parses enclosed string and replaces it with argument's <code>get()</code> value.
	 ...
	 * @param rmark Right mark of to-be-skipped block or null if does not exist (// comment). private void process(String pattern, String lmark,
	 * String rmark) { int idx = 0; while (true) { int ridx = -1, lidx = pattern.indexOf(lmark,idx); if (lidx >= 0) { if (rmark != null) { ridx =
	 * pattern.indexOf(rmark,lidx + lmark.length()); } else ridx = pattern.length(); } else break; if (ridx >= 0) { skipped.put(new Range(lidx,
	 * ridx-lidx)); if (rmark != null) idx = ridx+rmark.length(); else break; } else break; } }
	 */
	/**
	 * Returns the value for given key. Subclass may define its own beahvior of this method. For example, if key is not defined, subclass can return
	 * <not defined> string.
	 * @param key Key.
	 ...
	 * @exception IllegalArgumentException if number of arguments exceeds BUFSIZE or parser found unmatched brackets (this exception should be
	 * switched off using setExactMatch(false)).
	 */
	public String processPattern(String newPattern) throws IllegalArgumentException {
		...
		while (true) {
			...
			/*
			 * Range ran = skipped.getRangeContainingOffset(lidx); if (ran != null) { outpat.append(newPattern.substring(idx, ran.getEnd())); idx =
			 * ran.getEnd(); continue; }
			 */
			if (lidx >= 0) {
	...
	/**
	 * Test whether formatter will throw exception if object for key was not found. If given map does not contain object for key specified, it could
	...
	/**
	 * Specify whether formatter will throw exception if object for key was not found. If given map does not contain object for key specified, it
	 * could throw an exception. If does not throw, key is left unchanged.
	...
	/**
	 * Sets argument map This map should contain key-value pairs with key values used in formatted string expression. If value for key was not found,
	 * formatter leave key unchanged (except if you've set setThrowExceptionIfKeyWasNotFound(true), then it fires IllegalArgumentException.
